 ...
 * gitigonre : gitigonre파일은 Git에서 추적하지 않을 파일이나 디렉토리를 지정하는 데 사용됩니다. 이 파일을 사용하여 Git으로 관리도지 않아야 하는 로그, 빌드, 개인설정파일 등을 지정할 수 있습니다. 표준 Gold패턴을 사용합니다. 표준 Gold패턴이란 정규표현식을 단순화한 것입니다.
 * 사용법
    - vim .gitignore를 사용하여 파일을 생성합니다. 
    - 파일에 무시할 내용들을 추가합니다. 
